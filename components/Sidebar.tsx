...
import { LayoutDashboard, BookOpen, Bot, Scale, LogOut, GraduationCap, FileText, X, Moon, Sun } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, isOpen, onClose, isDarkMode, toggleTheme }) => {
  const navItems = [
    ...
    { view: ViewState.LEARNING, label: 'Doutrina & Aulas', icon: GraduationCap },
    { view: ViewState.EXAMS, label: 'Provas & OAB', icon: FileText },
    { view: ViewState.AI_TUTOR, label: 'Tutor Jurídico', icon: Bot },
  ];
      ...
      >
        <div className="p-6 flex items-center justify-between">
          <div className="flex items-center space-x-2 text-primary-600 dark:text-primary-400">
            {/* Ícone da Balança (Scale) para Direito */}
            <Scale className="w-8 h-8 fill-current" />
            <span className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Jus<span className="text-secondary-900 dark:text-primary-400">Mind</span></span>
          </div>
