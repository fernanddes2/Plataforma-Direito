...
const SkeletonLoader = () => (
  <div className="animate-pulse space-y-8 max-w-4xl mx-auto">
    <div className="flex items-center justify-center space-x-2 mb-8 text-primary-600 dark:text-primary-400">
      <Scale className="w-5 h-5 animate-spin-slow" />
      <span className="text-sm font-medium">O JusMind está consultando a doutrina e a legislação...</span>
    </div>
...
const LearningMode: React.FC<LearningModeProps> = ({ learningProgress, onUpdateProgress }) => {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('jusMindRecentModules');
      if (saved) {
  ...
  const updateRecentHistory = (module: LearningModule) => {
    setRecentModules(prev => {
      ...
      localStorage.setItem('jusMindRecentModules', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const handleOpenModule = async (module: LearningModule) => {
    ...
    const cacheKey = `jusMind_content_${module.id}`;
  ...
  const getSubjectStyle = (name: string) => {
    ...
    if (lower.includes('constitucional') || lower.includes('estado') || lower.includes('público') || lower.includes('administrativo')) {
        return { 
    ...
    if (lower.includes('civil') || lower.includes('família') || lower.includes('consumidor') || lower.includes('empresarial')) {
        return { 
    ...
    if (lower.includes('trabalho') || lower.includes('processo')) {
        return { 
  ...
  return (
    <div className="p-8 2xl:p-12 max-w-screen-2xl 2xl:max-w-[1800px] mx-auto">
      <div className="mb-8">
        <h1 className="text-3xl 2xl:text-4xl font-bold text-gray-900 dark:text-white">Doutrina e Aulas</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-2 2xl:text-lg">Guias de estudo gerados por IA baseados na grade oficial da UFF.</p>
        ...
        <div className="mt-6 relative max-w-xl">
            ...
            <input
                ...
                placeholder="Buscar disciplina (ex: Introdução ao Direito, Civil...)"
                value={searchTerm}
            ...
            >
                ...
                <div className="mt-4 pt-4 border-t border-gray-50 dark:border-slate-700">
                    ...
                    >
                        ...
                        {progress > 0 ? 'Continuar Leitura' : 'Iniciar Módulo'}
                    </button>
